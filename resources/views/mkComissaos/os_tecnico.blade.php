                          ...
                          <table class="table table-bordered" id="ordenacao">
                            ...
                            <tbody>
                                @foreach ($servicos as $servicos)
                                    <tr>
                                      <td>{{ \Carbon\Carbon::parse($servicos->data_fechamento)->format('d/m') }}</td>
                                      <td>{{ $servicos->codos }}</td>
                                      <td>@isset($servicos->usuario->nome_razaosocial) {{ $servicos->usuario->nome_razaosocial }} @endisset</td>
                                      <td>{{ $servicos->osTipo->descricao }}</td>
                                      <td>{{ $servicos->tecnico->usr_nome }}</td>
                                      <td>{{ $servicos->vendedor }}</td>
                                      <td>{{ $servicos->plano }}</td>
                                      <td>{{ $servicos->indicacoes }}</td>
                                      <td>{{ $servicos->classEncerramento->classificacao }} </td>
                                    </tr>
                                @endforeach
                            </tbody>
                          </table>
