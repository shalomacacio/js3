        ...
        <div class="invoice p-3 mb-3">
          ...
          <div class="row invoice-info">
            <div class="col-sm-4 invoice-col">
              Data Inicial
              <address>
                <strong>{{ \Carbon\Carbon::now()}}</strong><br>
              </address>
            </div>
            <!-- /.col -->
            <div class="col-sm-4 invoice-col">
          ...
          <!-- /.row -->
          <center><h4>CLIENTES X NASPORT </h4></center>
          <br/>
          <!-- Table row -->


        <div class="col-12">
        <p class="lead"><b>Total: {{ $radaccts->count() }} </b></p>
        </div>
          ...
          <div class="row">
            <div class="col-12 table-responsive">
              <table class="table table-striped table-sm " id="tblData" >
                ...
                <tr>
                  <td> {{ $radacct->conexao->username }} </td>
                  <td> {{ $radacct->conexao->cliente->nome_razaosocial }} </td>
                  <td> 
                      @isset($radacct->conexao->endereco->logradouro)
                        {{ $radacct->conexao->endereco->logradouro }} , {{ $radacct->conexao->numero }}  
                      @endisset  
                  </td>
                  <td> 
                    @isset($radacct->conexao->bairroConex->bairro)
                        {{ $radacct->conexao->bairroConex->bairro }} 
                    @endisset  
                  </td>
                  <td> 
                    @isset($radacct->conexao->cidadeConex->cidade)
                        {{ $radacct->conexao->cidadeConex->cidade }} 
                    @endisset  
                  </td>
                  <td> {{ $radacct->conexao->cliente->fone01 }} </td>
                  <td> {{ $radacct->conexao->cliente->fone02 }} </td>
                  <td> 
                      @isset($radacct->conexao->contratoAtual->planoAcesso)
                      {{ $radacct->conexao->contratoAtual->planoAcesso->descricao }} 
                      @endisset
                  </td>
                  <td> 
                    @isset($radacct->conexao->contratoAtual->vlr_renovacao)
                      {{ $radacct->conexao->contratoAtual->vlr_renovacao }} 
                    @endisset  
                </td> 
                @endforeach
                </tbody>
