...
<section class="content-header">
    ...
    <div class="container-fluid">
      <div class="row ">
        ...
        <div class="col-sm-10">
          <form class="form-inline"  action="{{ route('financeiro.movCaixa') }}"   method="GET">
            @csrf
...
<section class="content">
    <div class="container-fluid">
        ...
        <center><h4> MOVIMENTAÇÕES CAIXA  </h4></center>
                ...
                <div class="col-12">
                    ...
                    <table id="example" class="table table-striped table-sm  display nowrap" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td> {{ $mov->codmovcaixa }} </td> 
                                <td> {{ $mov->codfatura }} </td> 
                                <td> {{ $mov->codvinculado }}  </td>
                                <td> {{ $mov->codconta }}  </td>
                                <td> {{ $mov->nome_razaosocial }} </td>
                                <td> {{ $mov->descricao_conta }}  </td>
                                <td> {{ $mov->cidade }} </td>
                                <td> {{ \Carbon\Carbon::parse($mov->data_vencimento)->format('d-m-Y') }} </td>
                                <td> {{ $mov->valor_original }} </td>
                                <td> {{ $mov->suspenso }} </td>
                                <td> {{ $mov->vlr_liquidacao }} </td>
                                <td> {{ Str::upper($mov->usuario_liquidacao) }} </td>
                                <td> {{ \Carbon\Carbon::parse($mov->data_liquidacao)->format('d-m-Y')  }} </td>
                                <td> {{ $mov->unidadefin }}  </td>
                                <td> {{ $mov->unidade }} </td>
                                <td> {{ $mov->forma_pgto_liquidacao }} </td>
                                <td> {{ $mov->suspenso }} </td>
                            </tr>
                            @endforeach
                        </tbody>
